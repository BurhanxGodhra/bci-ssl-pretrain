# tests/test_accuracy_sprint_flat_lr_sweep.py
import json
...
device = get_device()
CHECKPOINT = "checkpoints/encoder_bnci2014_001_only_e25_flat.pt"
...
print("\n=== Flat-LR vs Cosine-LR (both BNCI-only) ===")
cosine_ref = {1: 0.3271, 5: 0.4073, 10: 0.4385, 20: 0.4602}
aggregate = {}
for k in K_VALUES:
    ...
    delta = agg - cosine_ref[k]
    print(f"  k={k:2d}: flat-lr={agg:.4f}  cosine-lr={cosine_ref[k]:.4f}  delta={delta:+.4f}")

out_path = Path("results/phase5_fewshot_linear_probe_bnci_only_flat_lr.json")
with open(out_path, "w") as f:
    json.dump({
        "dataset": "bnci2014_001_only_flat_lr", "checkpoint": CHECKPOINT,
        ...
        "cosine_lr_reference": cosine_ref,
    }, f, indent=2)
print(f"\nSaved -> {out_path}")