...
**D-016 — Dependencies installed manually during development never made it into `requirements.txt`**
*Final verification pass.* `streamlit`, `onnx`, `onnxruntime`, and `onnxscript` were each installed via standalone `pip install` commands at various points (Streamlit for the apps, the ONNX trio for the export script) and never added to the tracked dependency file — the same class of gap as an earlier, separately-caught `pylsl` omission. The first "fresh clone" verification attempt gave a false pass because it ran with both a project venv and conda's base environment simultaneously active, letting `streamlit` resolve via PATH from base rather than the venv actually built from `requirements.txt`. Caught by re-running the verification with conda base fully deactivated, which failed loudly (`command not found: streamlit`) exactly as it should have the first time. Fixed by adding all four packages to `requirements.txt` and re-verifying in a properly isolated environment.

**D-017 — Consistent, friendly error handling for missing checkpoints across all app pages**
*Final verification pass.* The Benchmarking Lab already caught a missing-checkpoint case with a clear message; the Benchmark Demo page did not, surfacing a raw Python traceback instead. Since Benchmark Demo is the first page in navigation, a stranger's first interaction with the project could have been a wall of red text rather than clear instructions. Fixed by adding the same existence-check-and-friendly-message pattern used in the Benchmarking Lab.